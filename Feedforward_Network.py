    ...
    def train(self, X, y, repeat = 1):
        for j in range(0, repeat):
            for i, Xi in enumerate(X):
                ...
                l1.shape = (l1.shape[0], 1)
                l0.shape = (l0.shape[0], 1)
                self.syn1 += l1.dot(l2_delta.reshape(1,1))
                self.syn0 += l0.dot(l1_delta.reshape(l1_delta.shape[0], 1).T)
